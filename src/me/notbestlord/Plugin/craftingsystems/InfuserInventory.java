package me.notbestlord.Plugin.craftingsystems;

import java.util.ArrayList;
import java.util.Dictionary;
import java.util.Hashtable;
import java.util.List;
import java.util.UUID;

import org.bukkit.Bukkit;
import org.bukkit.ChatColor;
import org.bukkit.Material;
import org.bukkit.entity.Player;
import org.bukkit.event.inventory.InventoryType;
import org.bukkit.inventory.Inventory;
import org.bukkit.inventory.ItemStack;
import org.bukkit.inventory.meta.ItemMeta;

public class InfuserInventory {
	
	public static Dictionary<UUID, Inventory> inventory = new Hashtable<>();
	public static Inventory inv;
	public static void createInfuser(Player player) {
		inv = Bukkit.createInventory(null, 27, ChatColor.AQUA+"Infuser");
		ItemStack item1 = new ItemStack(Material.GRAY_STAINED_GLASS_PANE);
		ItemMeta meta = item1.getItemMeta();
		meta.setDisplayName(" ");
		item1.setItemMeta(meta);
		
		inv.setItem(0, item1);
		inv.setItem(1, item1);
		inv.setItem(2, item1);
		inv.setItem(3, item1);
		inv.setItem(4, item1);
		inv.setItem(6, item1);
		inv.setItem(7, item1);
		inv.setItem(8, item1);
		inv.setItem(9, item1);
		inv.setItem(13, item1);
		inv.setItem(15, item1);
		inv.setItem(18, item1);
		inv.setItem(19, item1);
		inv.setItem(20, item1);
		inv.setItem(21, item1);
		inv.setItem(22, item1);
		inv.setItem(24, item1);
		inv.setItem(25, item1);
		inv.setItem(26, item1);
		
		item1.setType(Material.WHITE_STAINED_GLASS_PANE);
		inv.setItem(5, item1);
		inv.setItem(23, item1);
		
		item1.setType(Material.LIME_STAINED_GLASS_PANE);
		meta.setDisplayName(ChatColor.WHITE+"Use Infuser");
		item1.setItemMeta(meta);
		inv.setItem(14, item1);
		
		item1.setType(Material.BARRIER);
		meta.setDisplayName(ChatColor.RED+"Close");
		item1.setItemMeta(meta);
		inv.setItem(17, item1);
		
		item1.setType(Material.BLUE_STAINED_GLASS_PANE);
		meta.setDisplayName(ChatColor.WHITE+"<- Item");
		List<String> lore = new ArrayList<>();
		lore.add(ChatColor.WHITE+"Infusing medium ->");
		meta.setLore(lore);
		item1.setItemMeta(meta);
		inv.setItem(11, item1);
		inventory.put(player.getUniqueId(), inv);
		
		
	}
}
